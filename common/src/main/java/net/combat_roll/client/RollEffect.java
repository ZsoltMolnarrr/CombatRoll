...
public record RollEffect(Visuals visuals, String soundId) {
    ...
    public static void playVisuals(Visuals visuals, PlayerEntity player, Vec3d direction) {
        ...
        if (CombatRollClient.config.playRollSound) {
            ...
            if (sound != null) {
                player.getEntityWorld().playSoundClient(player.getX(), player.getY(), player.getZ(), sound, SoundCategory.PLAYERS, 1, 1, true);
            }
        }
        switch (visuals.particles()) {
            case PUFF -> {
                for(int i = 0; i < 15; ++i) {
                    ...
                    player.getEntityWorld().addParticleClient(ParticleTypes.POOF,
